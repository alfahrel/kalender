...
class CalendarAdapter(private val days: List<CalendarDay>) :
    ...
    inner class DayVH(view: View) : RecyclerView.ViewHolder(view) {
        val tvDay: TextView = view.findViewById(alfahrel.my.id.kalender.R.id.tvDay)
        val dot: View       = view.findViewById(alfahrel.my.id.kalender.R.id.dotHoliday)
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) =
        DayVH(LayoutInflater.from(parent.context)
            .inflate(alfahrel.my.id.kalender.R.layout.item_calendar_day, parent, false))
    ...
    override fun onBindViewHolder(h: DayVH, position: Int) {
        ...
        when {
            item.isToday -> {
                h.tvDay.setBackgroundResource(alfahrel.my.id.kalender.R.drawable.bg_today)
                h.tvDay.setTextColor(ctx.resolveAttrColor(R.attr.colorOnPrimary))
            }
            item.holidayType == HolidayType.JOINT_LEAVE -> {
                h.tvDay.setBackgroundResource(alfahrel.my.id.kalender.R.drawable.bg_joint)
                h.tvDay.setTextColor(ctx.resolveAttrColor(R.attr.colorOnErrorContainer))
            }
            item.isHoliday -> {
                h.tvDay.setBackgroundResource(alfahrel.my.id.kalender.R.drawable.bg_holiday)
                h.tvDay.setTextColor(ctx.resolveAttrColor(R.attr.colorOnError))
            }
            item.isSunday -> {
                h.tvDay.setBackgroundResource(alfahrel.my.id.kalender.R.drawable.bg_sunday)
                h.tvDay.setTextColor(ctx.resolveAttrColor(R.attr.colorOnError))
            }
            else -> {
                h.tvDay.background = null
